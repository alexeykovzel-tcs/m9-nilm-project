...
class Tag:
    ...
    def find_power_cycle(self, cycles):
        """ finds a power cycle that matches the tag cycle. """
        power, cycle = max(cycles, key=lambda x: self._overlap_score(x[1]))
        return power.truncate(cycle)

    def _overlap_score(self, cycle):
        """ calculate how the given cycle overlaps with the tag cycle. """
        on, off = cycle
...
class MeterData:
    ...
    def tagged_cycles(self):
        return [] if not self.cycles else [
            (tag, cycle) for tag in [tag.shift_cycle() for tag in self.tags]
            for cycle in [tag.find_power_cycle(self.cycles)] if cycle is not None
        ]
